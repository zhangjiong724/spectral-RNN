...
// CUDA kernel TODO
__global__ void SvdProdGpuKernel(const float* H, const float* U, float* H_out, const int n_h, const int batch, const int n_r) {
  //for (int i = 1; i < N; i++) t_out(i) = 0;
  //T_out(0) = T_in(0);
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n_h*batch; i += blockDim.x * gridDim.x) {
    	H_out[i] = 2.0 ;
  	}
}
...
int SvdProdGpuKernelLauncher(const float* H, const float* U, float* H_out, const int n_h, const int batch, const int n_r) {
	...
	if (stat != CUBLAS_STATUS_SUCCESS) { printf ("CUBLAS initialization failed on SvdProd\n"); return EXIT_FAILURE; }
	// allocate alpha
	float* alpha;
