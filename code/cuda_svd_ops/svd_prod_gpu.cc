...
int SvdProdGpuKernelLauncher(const float* H, const float* U, float* H_out, const int n_h, const int batch, const int n_r);

class SvdProdGpuOp : public OpKernel {
public:
  explicit SvdProdGpuOp(OpKernelConstruction* context) : OpKernel(context) {}

  void Compute(OpKernelContext* context) override {
	// Check number of inputs
	OP_REQUIRES(context, context->num_inputs() == 2,
                errors::InvalidArgument("SvdProd expects 2 inputes."));
...
#if GOOGLE_CUDA
	int op_status;
    op_status = SvdProdGpuKernelLauncher(H_data, U_data, H_out_data, n_h, batch, n_r );
#endif
  }
};

REGISTER_KERNEL_BUILDER(Name("SvdProdGpu").Device(DEVICE_GPU), SvdProdGpuOp);
