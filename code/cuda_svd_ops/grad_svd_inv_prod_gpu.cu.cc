...
int GradSvdInvProdGpuKernelLauncher(const float* H, const float* U, const float* G, float* H_grad, float* U_grad, const int n_h, const int batch, const int n_r) {
	cublasStatus_t stat;
	cudaError_t cudaStat;
	cublasHandle_t handle;
	// creat handle
	stat = cublasCreate_v2(&handle);
	if (stat != CUBLAS_STATUS_SUCCESS) { printf ("CUBLAS initialization failed on GradSvdInvProd\n"); return EXIT_FAILURE; }
	// allocate alpha
	float* alpha;
	...
	// begin computation
	Hprod(handle, H, H_hist + (n_r-2)*batch*n_h, U + n_h*(n_r-1), alpha, n_h - n_r + 1, n_h, batch);
	for(int r=n_r-2; r > 0; r--) {
		Hprod(handle, H_hist + r*batch*n_h, H_hist + (r-1)*batch*n_h, U + n_h*r, alpha, n_h - r, n_h, batch);
	}

	stat = cublasScopy(handle, n_h * batch , G, 1, H_grad, 1); // fill H_out with H_in

	for(int r=0; r < n_r-1; r++) {
		Hgrad(handle, H_hist + r*batch*n_h, U + n_h*r, H_grad, U_grad + n_h*r, alpha, beta, n_h - r, n_h, batch);
	}
	Hgrad(handle, H, U + n_h*(n_r-1), H_grad, U_grad + n_h*(n_r-1), alpha, beta, n_h - n_r + 1, n_h, batch);
