...
int GradSvdProdGpuKernelLauncher(const float* H, const float* U, const float* G, float* H_grad, float* G_grad, const int n_h, const int batch, const int n_r);

class GradSvdProdGpuOp : public OpKernel {
public:
  explicit GradSvdProdGpuOp(OpKernelConstruction* context) : OpKernel(context) {}

  void Compute(OpKernelContext* context) override {
	// Check number of inputs
	OP_REQUIRES(context, context->num_inputs() == 3,
                errors::InvalidArgument("GradSvdProd expects 3 inputes."));
	...
	OP_REQUIRES(context, TensorShapeUtils::IsMatrix(H_shape),
                errors::InvalidArgument("SvdProd expects H to be a 2-D matrix."));
	OP_REQUIRES(context, TensorShapeUtils::IsMatrix(U_shape),
                errors::InvalidArgument("SvdProd expects U to be a 2-D matrix."));
	OP_REQUIRES(context, TensorShapeUtils::IsMatrix(G_shape),
                errors::InvalidArgument("SvdProd expects G to be a 2-D matrix."));
...
#if GOOGLE_CUDA
    GradSvdProdGpuKernelLauncher(H_data, U_data, G_data, Grad_H_data, Grad_U_data, n_h, batch, n_r);
#endif
  }
};

REGISTER_KERNEL_BUILDER(Name("GradSvdProdGpu").Device(DEVICE_GPU), GradSvdProdGpuOp);
